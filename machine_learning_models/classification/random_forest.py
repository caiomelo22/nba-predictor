...
dataset = pd.read_csv('Winner_2000-2020.csv')
X = dataset.iloc[:, 4:-1]
y = dataset.iloc[:, -1]
# X = dataset.iloc[:int(len(dataset.values)*0.8), 1:-1].values
# y = dataset.iloc[:int(len(dataset.values)*0.8), -1].values
# X_valid = dataset.iloc[int(len(dataset.values)*0.2):, 1:-1].values
# y_valid = dataset.iloc[int(len(dataset.values)*0.2):, -1].values
...
top_corr_features_a = corrmat.index
...
top_corr_features_b = corrmat.index
plt.figure(figsize=(13,13))
#plot heat map
...
Predicting Both Test Set and Validation Set
"""
